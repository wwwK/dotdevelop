...
MonoDevelop is split in several tarballs: one for the main application, and one for each optional add-in. You can get the tarballs from the [Download](https://download.mono-project.com/sources/monodevelop/) page.
...
Building MonoDevelop from source on the Mac is straightforward when the latest **Mono SDK** package is installed. In general, the instructions in [Getting Started](https://github.com/mono/monodevelop/wiki/Getting-Started) can be applied directly to building MD on OS X. However, there are a few caveats, so this page explains the Mac build process in more detail.
